...
- **Windows specific**:
  - Add `C:\src\flutter\bin` and `C:\src\flutter\bin\cache\dart-sdk\bin` to your `PATH`, otherwise commands such as `flutter doctor` and `dart` are unavailable from PowerShell.
  - Install Visual Studio 2022 with the *Desktop development with C++* workload so that `flutter build windows` has the required MSVC toolchain.
