...
mixin OpenIMLive {
  ...
  final _callRecordHistory = <String>{};
  ...
  final _certificateCache = <String, Future<SignalingCertificate>>{};
        ...
        (event) async {
          ...
          if (event.state == CallState.beCalled) {
            if (OpenIMLiveClient().isBusy) {
              _sendBusyLine(event.data);
              return;
            }
            _preFetchCertificate(event.data);
            _playSound();
            ...
            if (Platform.isAndroid && _isRunningBackground) {
              ...
              _showIncomingCallNotification(event.data);
              return;
            }
            _beCalledEvent = null;
  ...
  _insertSignalingMessageListener() {
    insertSignalingMessageSubject.listen((value) {
      if (!_shouldInsertCallRecord(value)) {
        return;
      }
      final key = _buildCallRecordKey(value);
      if (_callRecordHistory.contains(key)) {
        return;
      }
      _callRecordHistory.add(key);
      _insertMessage(
        state: value.state,
  ...
  String _buildCallRecordKey(CallEvent event) {
    final invitation = event.data.invitation;
    final inviter = invitation?.inviterUserID ?? '';
    final invitee = invitation?.inviteeUserIDList?.first ?? '';
    final room = invitation?.roomID ?? '';
    final normalizedState = _normalizeCallState(event.state).name;
    return '$normalizedState-$inviter-$invitee-$room';
  }

  CallState _normalizeCallState(CallState state) {
    switch (state) {
  ...
  bool _shouldInsertCallRecord(CallEvent event) {
    switch (event.state) {
  ...
  Future<void> _sendBusyLine(SignalingInfo info) async {
    final invitation = info.invitation;
    if (invitation == null) return;
    Logger.print('busy line triggered for ${invitation.inviterUserID}');
    ...
    try {
      final custom = await OpenIM.iMManager.messageManager.createCustomMessage(
        data: jsonEncode({
        ...
        await OpenIM.iMManager.messageManager.sendMessage(
          message: custom,
          offlinePushInfo: OfflinePushInfo(title: StrRes.busyLine),
        userID: invitation.inviterUserID,
        isOnlineOnly: true,
      );
    } catch (e, s) {
      Logger.print('send busy line failed: $e\n$s');
    }
    onBusyLine();
  }
  ...
  Future<SignalingCertificate> onTapPickup(SignalingInfo signaling) async {
    ...
    final certificate = await _consumeCertificate(roomID);
    return certificate;
  }
  ...
  onTapReject(SignalingInfo signaling) async {
    ...
    _clearCachedCertificate(signaling);
  ...
  onTapHangup(SignalingInfo signaling, int duration, bool isPositive) async {
    ...
    final message =
        await OpenIM.iMManager.messageManager.createCustomMessage(data: jsonEncode(data), extension: '', description: '');
    final recvUserID = signaling.invitation!.inviterUserID == OpenIM.iMManager.userID
        ? signaling.invitation!.inviteeUserIDList!.first
        : signaling.invitation!.inviterUserID;
    OpenIM.iMManager.messageManager
    ...
    insertSignalingMessageSubject.add(CallEvent(
      isPositive ? CallState.hangup : CallState.beHangup,
      signaling,
      fields: duration,
    ));
  }
  ...
  void _showIncomingCallNotification(SignalingInfo info) {
    PackageBridge.callNotificationBridge?.showIncomingCallNotification(info);
  }

  void _dismissIncomingCallNotification() {
    PackageBridge.callNotificationBridge?.cancelIncomingCallNotification();
  }
  ...
  void _preFetchCertificate(SignalingInfo signaling) {
    final roomID = signaling.invitation?.roomID;
    if (roomID == null || roomID.isEmpty) return;
    _certificateCache.putIfAbsent(roomID, () {
      Logger.print('prefetch RTC token for room: $roomID');
      return Apis.getTokenForRTC(roomID, OpenIM.iMManager.userID);
    });
  }

  Future<SignalingCertificate> _consumeCertificate(String roomID) {
    final cached = _certificateCache.remove(roomID);
    if (cached != null) {
      Logger.print('consume prefetch token for room: $roomID');
      return cached;
    }
    Logger.print('token cache miss, fetch immediately for room: $roomID');
    return Apis.getTokenForRTC(roomID, OpenIM.iMManager.userID);
  }

  void _clearCachedCertificate(SignalingInfo signaling) {
    final roomID = signaling.invitation?.roomID;
    if (roomID == null) return;
    _certificateCache.remove(roomID);
  }
  ...
  void _insertMessage({
    ...
    (() async {
      final invitation = signalingInfo.invitation;
      if (invitation == null) {
        Logger.print('skip insert call record: empty invitation', functionName: '_insertMessage');
        return;
      }
      final mediaType = invitation.mediaType;
      final inviterUserID = invitation.inviterUserID;
      final inviteeUserID = invitation.inviteeUserIDList?.firstOrNull;
      final groupID = invitation.groupID;
      final sessionType = invitation.sessionType ?? ConversationType.single;
      if (inviterUserID == null || inviterUserID.isEmpty) {
        Logger.print('skip insert call record: inviter empty', functionName: '_insertMessage');
        return;
      }
      Logger.print(
      ...
      final message = (await OpenIM.iMManager.messageManager.createCallMessage(
        state: state.name,
        type: mediaType!,
        duration: duration,
      ))
      ...
      if (sessionType == ConversationType.group || sessionType == ConversationType.superGroup) {
        if (groupID == null || groupID.isEmpty) {
          Logger.print('skip insert call record: groupID empty for group session', functionName: '_insertMessage');
          return;
        }
        inserted = await OpenIM.iMManager.messageManager.insertGroupMessageToLocalStorage(
          groupID: groupID,
        ...
        final receiverID = inviterUserID == OpenIM.iMManager.userID ? inviteeUserID : inviterUserID;
        if (receiverID == null || receiverID.isEmpty) {
          Logger.print('skip insert call record: receiver empty for single session', functionName: '_insertMessage');
          return;
        }
        inserted = await OpenIM.iMManager.messageManager.insertSingleMessageToLocalStorage(
      ...
      if (inserted != null) {
        onSignalingMessage?.call(SignalingMessageEvent(
          inserted,
          sessionType,
          userID,
          targetGroupID,
