...
class _ChatPictureViewState extends State<ChatPictureView> {
  ...
  void initState() {
    ...
    final sourceUrl = picture?.sourcePicture?.url;
    _sourceUrl = IMUtils.resolveMediaUrl(picture?.bigPicture?.url) ?? IMUtils.resolveMediaUrl(sourceUrl);
    final snap = picture?.snapshotPicture?.url;
    _snapshotUrl = IMUtils.resolveThumbnailUrl(
      snap,
      fallback: _sourceUrl ?? IMUtils.resolveMediaUrl(sourceUrl),
    );
