...
class BottomSheetView extends StatelessWidget {
          ...
          children: [
            Container(
              ...
              child: Column(
                mainAxisSize: MainAxisSize.min,
                children: items
                    .map((item) => _parseItem(context, navigator, item))
                    .toList(),
              ),
            ),
            10.verticalSpace,
  ...
  Widget _parseItem(
      BuildContext context, NavigatorState navigator, SheetItem item) {
    ...
    return _itemBgView(
      ...
      onTap: () {
        if (!isOverlaySheet) {
          if (Get.overlayContext != null) {
            Get.back(result: item.result);
          } else {
            navigator.maybePop(item.result);
          }
        }
        item.onTap?.call();
      },
