...
class IMViews {
  ...
  static Future showToast(
    ...
    if (msg.trim().isEmpty) return Future.value();
    final bool? previous = EasyLoading.instance.userInteractions;
    final bool shouldEnable = previous != true;
    if (allowUserInteraction && shouldEnable) {
      EasyLoading.instance.userInteractions = true;
    }
    try {
      return await EasyLoading.showToast(msg, duration: duration);
    } finally {
      if (allowUserInteraction && shouldEnable) {
        EasyLoading.instance.userInteractions = previous ?? false;
      }
    }
  }
  ...
  static TextSpan getTimelineTextSpan(int ms) {
    ...
    if (DateUtil.isToday(ms, locMs: locTimeMs)) {
      return TextSpan(
        text: languageCode == 'zh' ? '浠婂ぉ' : 'Today',
    ...
    if (DateUtil.isYesterdayByMs(ms, locTimeMs)) {
      return TextSpan(
        text: languageCode == 'zh' ? '鏄ㄥぉ' : 'Yesterday',
        style: Styles.ts_0C1C33_17sp_medium,
      );
    }
    ...
    if (DateUtil.isWeek(ms, locMs: locTimeMs)) {
      ...
      if (weekday.contains('鏄熸湡')) {
        return TextSpan(
          text: weekday.replaceAll('鏄熸湡', ''),
          style: Styles.ts_0C1C33_17sp_medium,
          children: [
            TextSpan(
              text: '\n鏄熸湡',
    ...
    final monthPart = parts.isNotEmpty ? parts[0] : '';
    final dayPart = parts.length > 1 ? parts[1] : '';
    return TextSpan(
      text: dayPart.isNotEmpty ? '${int.parse(dayPart)}' : '',
      style: Styles.ts_0C1C33_17sp_medium,
      children: [
        TextSpan(
          text: monthPart.isNotEmpty
              ? '\n${int.parse(monthPart)}${languageCode == 'zh' ? '月' : ''}'
              : '',
          style: Styles.ts_0C1C33_12sp_medium,
        ),
      ],
