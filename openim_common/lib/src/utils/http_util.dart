...
class HttpUtil {
  ...
  static Future post(
    ...
    try {
      ...
      Logger.print('[HttpUtil] POST $path headers=${options.headers}');
      ...
      Logger.print('[HttpUtil] POST $path respCode=${resp.errCode}');
      if (resp.errCode == 0) {
        return resp.data;
      } else {
        if (showErrorToast) {
      ...
      if (error is DioException) {
        ...
        final errorMsg = '接口 $path 请求失败：$detail';
        if (showErrorToast) IMViews.showToast(errorMsg);
        return Future.error(errorMsg);
      }
      final errorMsg = '接口 $path 请求异常：${error.toString()}';
      if (showErrorToast) IMViews.showToast(errorMsg);
