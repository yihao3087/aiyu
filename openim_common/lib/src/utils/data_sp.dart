...
class DataSp {
  ...
  static const _deviceProfileStatus = 'deviceProfileCompleted';
  ...
  static const _meetingInProgress = '%s_meetingInProgress';
  static const _secureImTokenKey = 'secure.imToken';
  static const _secureChatTokenKey = 'secure.chatToken';
  ...
  static String? _imTokenCache;
  static String? _chatTokenCache;

  DataSp._();

  static Future<void> init() async {
    await SpUtil().init();
    _imTokenCache = await _secureStorage.read(key: _secureImTokenKey);
    _chatTokenCache = await _secureStorage.read(key: _secureChatTokenKey);
  }
  ...
  static String? get imToken => _imTokenCache;

  static String? get chatToken => _chatTokenCache;

  static String? get userID => getLoginCertificate()?.userID;

  static Future<void> putLoginCertificate(LoginCertificate lc) async {
    final sanitized = lc.toJson()..remove('password');
    await SpUtil().putObject(_loginCertificate, sanitized);
    await _persistTokens(lc.imToken, lc.chatToken);
  }
  ...
  static Future<bool>? putLoginAccount(Map map) {
    ...
    return SpUtil().putObject(_loginAccount, sanitized);
  }

  static LoginCertificate? getLoginCertificate() {
    final cert = SpUtil().getObj(
      _loginCertificate,
      (v) => LoginCertificate.fromJson(v.cast()),
    );
    if (cert == null) return null;
    if (_imTokenCache?.isNotEmpty == true) {
      cert.imToken = _imTokenCache!;
    } else if (cert.imToken?.isNotEmpty == true) {
      _imTokenCache = cert.imToken;
    }
    if (_chatTokenCache?.isNotEmpty == true) {
      cert.chatToken = _chatTokenCache!;
    } else if (cert.chatToken?.isNotEmpty == true) {
      _chatTokenCache = cert.chatToken;
    }
    return cert;
  }

  static Future<void> removeLoginCertificate() async {
    await SpUtil().remove(_loginCertificate);
    await _clearTokens();
    putDeviceProfileStatus(true);
  }
  ...
  static Future<bool>? putDeviceProfileStatus(bool needProfile) {
    return SpUtil().putBool(_deviceProfileStatus, needProfile);
  }

  static bool get needCompleteProfile {
    return SpUtil().getBool(_deviceProfileStatus, defValue: true)!;
  }
  ...
  static String? getMeetingInProgress() {
    return SpUtil().getString(getKey(_meetingInProgress));
  }
  ...
  static Future<void> _persistTokens(String? imToken, String? chatToken) async {
    await Future.wait([
      _writeToken(_secureImTokenKey, imToken, (value) => _imTokenCache = value),
      _writeToken(_secureChatTokenKey, chatToken, (value) => _chatTokenCache = value),
    ]);
  }

  static Future<void> _clearTokens() async {
    await Future.wait([
      _writeToken(_secureImTokenKey, null, (value) => _imTokenCache = value),
      _writeToken(_secureChatTokenKey, null, (value) => _chatTokenCache = value),
    ]);
  }
  ...
  static Future<void> _writeToken(
    String key,
    String? value,
    void Function(String?) cacheSetter,
  ) async {
